...
const { User, Address, Wallet, Points, Rating, Order, Reservation } = require('../models');
...
class UserController {
  ...
  async getProfile(req, res) {
    try {
      ...
      const recentReservations = await Reservation.find({ user_id: userId })
        .select('id reservation_type status date time created_at')
        .sort({ created_at: -1 })
        .limit(5);
  ...
  async updateProfile(req, res) {
    try {
      ...
      if (filteredUpdates.phone) {
        if (!this.isValidPhoneNumber(filteredUpdates.phone)) {
          return res.status(400).json({
            success: false,
  ...
  async updateAddress(req, res) {
    try {
      ...
      if (updates.latitude || updates.longitude) {
        const { validateAddress } = require('../utils');
        const validation = validateAddress({ ...address.toJSON(), ...updates });
      ...
      if (updates.is_default) {
        await Address.updateMany(
          { user_id: userId, id: { $ne: addressId } },
          { is_default: false }
        );
      }
      ...
      const pointsData = await Points.aggregate([
        { $match: { user_id: userId } },
        {
          $group: {
            ...
            total_spent: { $sum: '$points_spent' },
            current_balance: { $sum: { $subtract: ['$points_earned', '$points_spent'] } }
          }
        }
      ]);

      const balance = {
        current_balance: pointsData.length > 0 ? pointsData[0].current_balance : user.points,
        total_earned: pointsData.length > 0 ? pointsData[0].total_earned : 0,
        total_spent: pointsData.length > 0 ? pointsData[0].total_spent : 0
      };
  ...
  async getPointsHistory(req, res) {
    try {
      ...
      const [points, total] = await Promise.all([
        Points.find({ user_id: userId })
          .sort({ created_at: -1 })
          .skip(skip)
          .limit(limit),
        
        Points.countDocuments({ user_id: userId })
      ]);
    ...
    try {
      ...
      const [orders, reservations] = await Promise.all([
        Order.find({ user_id: userId })
          .populate('business_id', 'business_name address')
          .sort({ created_at: -1 })
          .skip(skip)
          .limit(limit),
  ...
  async deleteAccount(req, res) {
    try {
      ...
      // Delete related data
      await Promise.all([
        Address.deleteMany({ user_id: userId }),
        Points.deleteMany({ user_id: userId }),
        ...
        // Reservation.deleteMany({ user_id: userId }),
      ]);
  ...
  async getUserPointsBalance(userId) {
    try {
      ...
      const balance = pointsData.length > 0 ? pointsData[0].current_balance : user.points;
